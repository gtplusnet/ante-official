...
export class BoardLaneService {
  ...
  async reorderBoardLanes(input: BoardLaneOrderDto | ReorderSingleLaneDto) {
    if ('names' in input) {
      ...
      return { message: 'Lane reordered successfully' };
    }
  }
    ...
    for (let i = 0; i < names.length; i++) {
      ...
      if (!boardLane) {
        throw new ForbiddenException(
          `BoardLane with name ${names[i]} not found.`,
        );
      }
  ...
  async getFirstBoardLane(): Promise<BoardLaneInterface> {
    const boardLane = await this.prisma.boardLane.findFirst({
  ...
  async boardLaneTable(query: TableQueryDTO, body: TableBodyDTO) {
    ...
    const list = await Promise.all(
      baseList.map((boardLane) => this.formatBoardLaneResponse(boardLane)),
    );
  ...
  async getBoardLaneById({ id }) {
    ...
    return this.formatBoardLaneResponse(boardLaneInformation);
  }
  ...
  async reOrderBoardLanes(containerArray): Promise<void> {
    await Promise.all(
      containerArray.map(async (item, index) => {
        ...
        if (!boardLane) {
          throw new ForbiddenException(
            `BoardLane with id ${item.id} not found.`,
          );
        }
  ...
  /**
   * Formats a board lane response according to the standard format
   */
