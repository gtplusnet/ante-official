...
export class RoleService {
  ...
  async populateDefaultRole() {
    // Prevent reset if any role is assigned to an employee for this company
    ...
    if (assignedEmployee) {
      throw new BadRequestException(
        'Cannot reset default roles while roles are assigned to employees.',
      );
    }
    // 1. Delete all roles for the company
    await this.prisma.role.deleteMany({
      where: { companyId: this.utility.companyId },
    });

    // 2. Delete all user levels for the company
    await this.prisma.userLevel.deleteMany({
      where: { companyId: this.utility.companyId },
    });

    // 3. Copy all default user levels (companyId: null) to the company
    const defaultUserLevels = await this.prisma.userLevel.findMany({
    ...
    // 4. Copy all default roles (companyId: null) to the company, retaining parent-child
    const defaultRoleList = await this.prisma.role.findMany({
      where: { companyId: null },
    });
    // First pass: create all roles with parentRoleId null, build mapping
    ...
    // Third pass: create RoleUserLevel associations for company roles
    for (const defaultRole of defaultRoleList) {
    ...
    return {
      message: 'Default role list and user levels populated successfully',
    };
  }
