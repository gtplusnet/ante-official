<template>
  ...
  <div class="page-head">
    <div class="title text-title-large">Default User Level Management</div>
    <div class="actions">
      <q-btn class="q-mr-sm text-label-large" @click="resetUserLevel" color="red" outline unelevated rounded>
  ...
  <div class="page-content q-mt-md">
    <g-table :is-row-action-enabled="true" ref="table" tableKey="developerUserLevel" apiUrl="/user-level/default/table" class="text-body-small">
      <template v-slot:row-actions="props">
        ...
        <q-btn rounded @click="deleteUserLevel(props.data)" no-caps color="red" outline class="text-label-medium">
...
const AddEditUserLevelDialog = defineAsyncComponent(() =>
  import('../Settings/dialogs/SettingsAddEditUserLevelDialog.vue')
);
const RoleTreeDialog = defineAsyncComponent(() =>
  import('../Settings/dialogs/SettingsRoleTreeDialog.vue')
);

export default {
  name: 'DeveloperUserLevels',
  components: {
    GTable,
  ...
  methods: {
    ...
    deleteUserLevel(data) {
      this.$q.dialog({
        title: 'Delete User Level',
        message: 'Are you sure you want to delete this user level?',
        ok: true,
        cancel: true,
      }).onOk(() => {
        api.delete(`/user-level/default/${data.id}`).then(() => {
    ...
    resetUserLevel() {
      this.$q.dialog({
        title: 'Reset Default User Levels',
        message: 'Are you sure you want to reset all default user levels? This action cannot be undone.',
        ok: true,
        cancel: true,
      }).onOk(() => {
        this.$q.loading.show();
        api.post('/user-level/default/request-reset-otp', {})
          .then(() => {
            this.$q.loading.hide();
            this.$q.dialog({
              ...
              this.$q.loading.show();
              api.post('/user-level/default/reset-default-with-otp', { otp })
                .then(() => {
                  this.$q.notify({ type: 'positive', message: 'Default user levels reset successfully.' });
                  this.$refs.table.refetch();
                })
                .catch((err) => {
