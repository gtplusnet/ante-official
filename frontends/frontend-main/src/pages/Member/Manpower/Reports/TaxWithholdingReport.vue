...
import TaxWithholdingTable from './components/TaxWithholdingTable.vue';
import { getGovernmentPaymentHistoryTax } from '../../../../utility/api.handler';
...
const fetchData = async () => {
  ...
  try {
    ...
    const response = await getGovernmentPaymentHistoryTax($q, params);
    contributions.value = (response as unknown as ContributionData[]) || [];
    pagination.value.rowsNumber = contributions.value.length;
  } catch (error) {
    console.error('Error fetching tax withholding:', error);
    $q.notify({
      type: 'negative',
      message: 'Failed to load tax withholding records',
      position: 'top'
    });
  } finally {
    loading.value = false;
