    ...
    <div class="report-container">
      ...
      <div>
        <div class="row items-center">
          <div class="col">
            <div class="text-title-large">SSS Contributions Report</div>
...
import SSSContributionTable from './components/SSSContributionTable.vue';
import { getGovernmentPaymentHistorySSS } from '../../../../utility/api.handler';
...
const fetchData = async () => {
  ...
  try {
    ...
    const response = await getGovernmentPaymentHistorySSS($q, params);
    ...
    $q.notify({
      type: 'negative',
      message: 'Failed to load SSS contributions',
      position: 'top'
    });
  } finally {
    loading.value = false;
    ...
    sorted.sort((a, b) => {
      for (const sortCol of sortColumns.value) {
        ...
        if (column === 'basis' || column === 'employeeShare' || column === 'employerShare' || column === 'amount' ||
            column === 'employeeShareRegular' || column === 'employeeShareMPF' || 
            column === 'employerShareRegular' || column === 'employerShareMPF' || column === 'employerShareEC') {
