<template>
  <q-dialog ref="dialog" persistent>
    <TemplateDialog minWidth="800px">
      ...
      <template #DialogTitle>
        <div>Import Deduction Configuration - {{ deductionConfig?.name || '' }}</div>
      </template>
          ...
          <div v-if="currentStage === 'initial'" class="column">
            <div class="q-pa-md bgColor q-mb-md rounded-borders">
              <p class="text-title-medium text-center ">
                Import {{ deductionConfig?.category === 'LOAN' ? 'Loan' : 'Deduction' }} Configuration
              </p>
                ...
                <q-banner class="bg-info text-white">
                  ...
                  <div class="text-body-medium">
                    ...
                    <ul class="q-my-sm q-pl-md">
                      <li>Employee Code</li>
                      <li v-if="deductionConfig?.category === 'LOAN'">Loan Amount</li>
                      <li>Monthly Amortization</li>
                      <li>Deduction Period ({{ deductionPeriodOptions.join(', ') }})</li>
                      <li>Effectivity Date (YYYY-MM-DD)</li>
                    </ul>
                  </div>
                </q-banner>
              </div>
            ...
            <div v-if="validationResults.errorRows > 0" class="q-mb-md">
              <q-banner class="bg-negative text-white">
                ...
                <template v-slot:action>
                  <GButton variant="text" color="white" label="View Errors" @click="viewErrors" class="text-label-large"/>
                </template>
              </q-banner>
            </div>
  ...
  <ManpowerDeductionImportHistoryDialog ref="historyDialog" />

  <!-- Import Errors Dialog -->
  <ManpowerDeductionImportErrorsDialog ref="errorsDialog" @review-warnings="handleReviewWarnings" />
</template>
...
export default {
  ...
  components: {
    GButton,
    ManpowerDeductionImportHistoryDialog,
  ...
  data: () => ({
    ...
    deductionPeriodOptions: ['FIRST_PERIOD', 'LAST_PERIOD', 'EVERY_PERIOD'],
  }),
  ...
  methods: {
    ...
    async uploadFile() {
      ...
      try {
        const response = await this.$api.post(
          `/hr-configuration/deduction/import/${this.deductionConfigurationId}/upload`,
          formData,
          {
            headers: {
    ...
    async validateData() {
      ...
      try {
        await this.$api.post(`/hr-configuration/deduction/import/${this.batchId}/validate`);
    ...
    async reviewWarnings() {
      ...
      try {
        const response = await this.$api.get(`/hr-configuration/deduction/import/${this.batchId}/status`);
    ...
    async approveAndProcess() {
      ...
      try {
        ...
        await this.$api.post(`/hr-configuration/deduction/import/${this.batchId}/approve`, {
    ...
    async processImport() {
      ...
      try {
        await this.$api.post(`/hr-configuration/deduction/import/${this.batchId}/process`);
    ...
    async downloadTemplate() {
      try {
        const response = await this.$api.get(
          `/hr-configuration/deduction/import/${this.deductionConfigurationId}/template`,
          {
            responseType: 'blob',
          }
        );
