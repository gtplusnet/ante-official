<template>
  ...
  <ManpowerAllowanceImportErrorsDialog ref="errorsDialog" />
</template>
  ...
  methods: {
    ...
    async fetchImportHistory() {
      ...
      try {
        ...
        const response = await this.$api.put('/hr-configuration/allowance/import/history', body, {
          params,
          headers: {
    ...
    async downloadErrorReport(batchId) {
      try {
        ...
        const response = await this.$api.get(`/hr-configuration/allowance/import/history/${batchId}/download-errors`, {
          responseType: 'blob',
        });
        ...
        link.href = url;
        link.download = `allowance_import_errors_${batchId}.xlsx`;
