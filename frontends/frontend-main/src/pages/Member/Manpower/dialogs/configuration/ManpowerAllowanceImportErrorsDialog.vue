  ...
  <q-dialog ref="dialog" persistent>
    <TemplateDialog minWidth="1200px" maxWidth="90vw">
      ...
      <template #DialogContent>
        ...
        <!-- Error List -->
        <q-scroll-area style="height: 500px" class="q-pa-md">
          <q-list separator>
                ...
                <template v-slot:header>
                  ...
                  <q-item-section side>
                    <div class="text-grey-7">
                      Amount: {{ formatCurrency(record.allowanceAmount) }}
                    </div>
                  </q-item-section>
                </template>
                ...
                <q-card>
                  <q-card-section>
                    ...
                    <div class="row q-mb-md">
                      <div class="col-12">
                        <div class="text-subtitle2 q-mb-sm">Allowance Details</div>
                        <div class="row q-gutter-md text-body2">
                          <div class="col-auto">
                            <strong>Employee Code:</strong> {{ record.employeeCode || 'N/A' }}
                          </div>
                          <div class="col-auto">
                            <strong>Amount:</strong> {{ formatCurrency(record.allowanceAmount) || 'N/A' }}
                          </div>
                          <div class="col-auto">
                            <strong>Period:</strong> {{ record.allowancePeriod || 'N/A' }}
                          </div>
                          <div class="col-auto">
                            <strong>Effectivity Date:</strong> {{ formatDate(record.effectivityDate) || 'N/A' }}
                          </div>
                          <div class="col-auto">
                            <strong>Pro-Rated:</strong> {{ record.proRatedAllowance ? 'Yes' : 'No' }}
                          </div>
                        </div>
                      </div>
                    </div>
...
export default {
  name: 'ManpowerAllowanceImportErrorsDialog',
  components: {
    TemplateDialog,
    GButton
  },
  ...
  data() {
    return {
      ...
      fieldOptions: [
        ...
        { label: 'Allowance Amount', value: 'allowanceAmount' },
        { label: 'Allowance Period', value: 'allowancePeriod' },
        { label: 'Effectivity Date', value: 'effectivityDate' },
        { label: 'Pro-Rated', value: 'proRatedAllowance' },
      ],
    };
  },
  ...
  methods: {
    ...
    async loadData() {
      try {
        ...
        // Get batch info
        const batchResponse = await this.$api.get(`/hr-configuration/allowance/import/history/${this.batchId}`);
        this.batchInfo = batchResponse.data;

        // Get error records
        const errorResponse = await this.$api.get(`/hr-configuration/allowance/import/history/${this.batchId}/errors`);
    ...
    async downloadErrorReport() {
      try {
        ...
        const response = await this.$api.get(`/hr-configuration/allowance/import/history/${this.batchId}/download-errors`, {
          responseType: 'blob',
        });
        ...
        link.href = url;
        link.download = `allowance_import_errors_${this.batchId}.xlsx`;
