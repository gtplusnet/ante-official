    ...
    <q-card class="md3-dialog-card">
      ...
      <q-card-section class="md3-dialog-header">
        <div class="row items-center">
          <div class="col">
            <div class="text-h6">Select Multiple Employee</div>
          </div>
          <div class="col-auto">
          ...
          >
            ...
            <template v-slot:body="props">
              <q-tr :props="props" class="cursor-pointer">
                ...
                <q-td key="employeeCode" :props="props" @click="toggleRowSelection(props.row)">
                  {{ props.row.employeeCode }}
                </q-td>
                <q-td key="fullName" :props="props" @click="toggleRowSelection(props.row)">
                  {{ props.row.accountDetails.fullName }}
                </q-td>
                <q-td key="position" :props="props" @click="toggleRowSelection(props.row)">
      ...
      <q-card-section class="md3-dialog-footer">
        <div class="row justify-end q-gutter-sm">
          ...
          <GButton
            v-if="selectMultipleEmployee?.name !== 'Payroll Approvers'"
...
export default {
  name: 'SelectMultipleEmployeeDeduction',
  components: {
    GButton,
    CustomBranchTreeSelect,
  },
  props: {
    selectMultipleEmployee: {
      type: Object as () => { url: string; name?: string } | null,
      required: false,
      default: null,
    },
  },
  ...
  setup(props, { emit }) {
    ...
    const selectedRowsMap = ref<Map<string, EmployeeDataResponse>>(new Map());
    const searchBox = ref('');
    const allEmployees = ref<EmployeeDataResponse[]>([]);
    const filteredEmployeesList = ref<EmployeeDataResponse[]>([]);
    const isLoading = ref(false);

    // Computed property to get selected rows as array
    const selectedRows = computed(() => Array.from(selectedRowsMap.value.values()));
    ...
    // Remove onMounted - we'll load data when dialog shows instead

    const fetchEmployeeList = () => {
      isLoading.value = true;

      // Build URL with filters
      const baseUrl = props.selectMultipleEmployee?.url;
      if (!baseUrl) {
        console.error('No URL provided for employee selection');
      ...
      if (selectedBranchIds.value.length > 0) {
        // Backend now supports comma-separated branch IDs
      ...
      api
        ...
        .then((response) => {
          ...
          // Only clear selections if we're initializing (first load)
    ...
    const applyFilters = () => {
      // Fetch from server with filters instead of filtering locally
      fetchEmployeeList();
    };
    ...
    const toggleSelectAll = async () => {
      if (selectAll.value) {
        selectedRowsMap.value.clear();
        filteredEmployeesList.value.forEach(row => {
          selectedRowsMap.value.set(row.accountDetails.id, row);
        });
        selectedEmployees.value = Array.from(selectedRowsMap.value.keys());
      } else {
        selectedRowsMap.value.clear();
        selectedEmployees.value = [];
      }
    ...
    const isRowSelected = (row: EmployeeDataResponse) => {
      return selectedRowsMap.value.has(row.accountDetails.id);
    };

    const toggleRowSelection = async (row: EmployeeDataResponse) => {
      const id = row.accountDetails.id;

      if (selectedRowsMap.value.has(id)) {
      ...
      // Update selectedEmployees immediately
      selectedEmployees.value = Array.from(selectedRowsMap.value.keys());

      // Update selectAll checkbox state
      const selectedCount = selectedRowsMap.value.size;
      if (selectedCount === 0) {
        selectAll.value = false;
      } else if (selectedCount === filteredEmployeesList.value.length && filteredEmployeesList.value.length > 0) {
    ...
    const submitSelection = () => {
      ...
      emit('add-selected-employees', selectedEmployees.value);
    };
