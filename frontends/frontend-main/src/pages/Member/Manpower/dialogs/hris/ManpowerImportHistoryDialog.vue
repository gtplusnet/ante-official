<template>
  ...
  <ManpowerImportErrorsDialog ref="errorsDialog" />
</template>
  ...
  methods: {
    ...
    async fetchImportHistory() {
      ...
      try {
        ...
        const response = await api.put('/hris/employee/import/history', body, {
          params,
          headers: {
    ...
    async downloadErrorReport(batchId) {
      try {
        ...
        const response = await api.get(`/hris/employee/import/history/${batchId}/download-errors`, {
          responseType: 'blob',
        });
        ...
        link.href = url;
        link.download = `import_errors_${batchId}.xlsx`;
