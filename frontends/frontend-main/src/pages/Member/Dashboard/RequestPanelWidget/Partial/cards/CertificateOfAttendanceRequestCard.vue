<template>
  <div class="request-item" :class="statusClass" @click="$emit('click')">
    <div class="row items-center">
      ...
      <div class="text-title-small q-ml-sm">Certificate of Attendance</div>
    </div>
    <div class="row items-center justify-between">
      <div>
        <span v-if="filing.timeIn || filing.timeOut" class="text-body-small">
          Duration: <span class="text-label-medium">{{ formatDateRange(filing.timeIn, filing.timeOut) }}</span>
        </span>
      </div>
      <div class="text-grey-6 text-label-small" v-if="filing.status?.key !== 'REJECTED'">{{ filing.timeAgo }}</div>
    </div>
    <div v-if="filing.status?.key === 'REJECTED' && filing.rejectReason" class="reject-reason row justify-between">
      <div>
        <strong class="text-label-medium">Rejection Reason:</strong>
        <span class="text-body-medium"> {{ truncate(filing.rejectReason, 'REJECTION_REASON') }}</span>
      </div>
      <div class="text-grey-6 text-body-medium">{{ filing.timeAgo }}</div>
    </div>
  </div>
</template>
