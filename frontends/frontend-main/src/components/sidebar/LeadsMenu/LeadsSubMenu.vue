<template>
  <div class="q-pa-sm">
    ...
    <div v-for="item in filteredNavList" :key="item.key" class="leads-submenu">
      <div
        class="leads-item row items-center justify-between"
      ...
      <div
        v-if="item.child && item.child.length > 0 && openKey === item.key"
        class="q-pb-sm"
      >
        <div
          v-for="child in item.child"
          :key="child.key"
          class="leads-item-dropdown row items-center justify-between"
          :class="child.key == activeNav ? 'active' : ''"
          @click.stop="navigate(child.key)"
        >
          <div class="row items-center">
            <div>
              <q-icon
                :name="getActiveIcon(child.icon, child.key == activeNav)"
                size="20px"
                class="menu-icon q-mr-sm"
              />
            </div>
