        ...
        <tr v-for="item in itemData" :key="item.rowId"
          ...
          <td>
            <q-select v-if="!itemReceiptId" v-model="item.selectedItem" @update:model-value="itemChange(item, true)"
              :options="item.itemOptions" class="select-item" dense borderless flat use-input></q-select>
            <span v-else class="amount">{{ item.itemName }} ({{ item.itemSku }})</span>
          </td>

          <!-- quantity -->
          <td>
            <q-input @update:model-value="itemChange(item)" class="input quantity" v-model="item.quantity" type="number"
              borderless dense></q-input>
          </td>

          <!-- stock count -->
          <td v-if="warehouseId">
            <span class="stock">{{ item.stockCount || '0' }}</span>
          </td>

          <!-- description -->
          <td>
            <q-input @update:model-value="itemChange(item)" class="input description" v-model="item.description"
              type="text" borderless dense></q-input>
          </td>

          <!-- rate -->
          <td>
            <q-input @update:model-value="itemChange(item)" class="input description" v-model="item.rate" type="text"
              borderless dense></q-input>
          </td>

          <!-- amount -->
          <td>
            <span class="amount">{{ item.amountFormatted }}</span>
          </td>

          <!-- delete button -->
          <td @click="deleteItem(item)" class="delete">
            <q-icon class="text-red" size="20px" name="close"></q-icon>
          </td>
        </tr>
      </tbody>
