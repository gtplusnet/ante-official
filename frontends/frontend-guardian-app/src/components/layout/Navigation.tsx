...
import { FiGrid, FiFileText, FiUsers, FiSettings, FiDollarSign, FiX, FiLogOut, FiUserPlus } from "react-icons/fi";
...
export const Navigation: React.FC<NavigationProps> = ({ isOpen, onClose, guardian, company }) => {
      ...
      >
        <div className="pt-safe-top h-full flex flex-col">
          ...
          <div className={`p-4 border-b border-primary-400 transition-all duration-500 ${isAnimating ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4"}`}>
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                {/* Company Logo */}
                {company?.logoUrl ? (
                  <img src={company.logoUrl} alt={`${company.name} logo`} className="w-12 h-12 object-contain rounded-full bg-white/10 p-1" />
                ) : (
                  <div className="w-12 h-12 bg-primary-300 rounded-lg flex items-center justify-center">
                    <span className="text-lg font-bold text-primary-800">{company?.name ? company.name.charAt(0).toUpperCase() : "S"}</span>
                  </div>
                )}
                <div>
                  <div className="text-xs font-bold">{company?.name || "School Portal"}</div>
                  <p className="text-xs text-primary-200">Guardian Portal</p>
                </div>
              </div>
              <button onClick={onClose} className="p-1 rounded-lg hover:bg-primary-600 transition-all duration-200 hover:rotate-90" aria-label="Close menu">
            ...
            <div className={`flex items-center gap-3 transition-all duration-700 delay-100 ${isAnimating ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4"}`}>
              <div className="w-12 h-12 bg-primary-300 rounded-full flex items-center justify-center overflow-hidden transition-transform duration-300 hover:scale-110">
                {guardian.photoUrl ? (
                  <img src={guardian.photoUrl} alt={guardian.name} className="w-full h-full object-cover" />
                ) : (
          ...
          <div className="flex-1 py-4">
            <p className={`px-4 text-xs text-primary-200 uppercase tracking-wider mb-2 transition-all duration-500 delay-200 ${isAnimating ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4"}`}>Menu</p>
              ...
              return (
                <Link
                  ...
                  className={`w-full text-left
                    ...
                    ${isActive ? "bg-primary-600 text-white border-l-4 border-secondary-500 nav-active-glow" : "text-primary-100 hover:bg-primary-600 hover:text-white hover:pl-6 border-l-4 border-transparent"}
                ...
                >
                  {isActive && <div className="absolute left-0 top-0 bottom-0 w-1 bg-secondary-500 animate-pulse" />}
          ...
          <div className={`p-4 border-t border-primary-400 transition-all duration-700 delay-500 ${isAnimating ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
            ...
            <Link href="/login" onClick={onClose} className="w-full flex items-center justify-center gap-2 text-primary-100 hover:text-white transition-all duration-200 hover:scale-105">
