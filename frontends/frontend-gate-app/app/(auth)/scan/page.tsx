...
import {
  getAttendanceAPIService,
  AttendanceRecord,
  AttendanceStats,
} from "@/lib/services/attendance-api.service";
...
export default function ScanPage() {
  ...
  useEffect(() => {
    ...
    if (initializationRef.current || initializingRef.current) {
      console.log(
        "🔒 [ScanPage] Already initialized or initializing, skipping..."
      );
      return;
    }
        ...
        if (typeof window !== "undefined") {
          ...
          (window as any).checkIndexes = async () => {
            try {
              ...
              console.log(
                "Student indexes:",
                Array.from(studentStore.indexNames)
              );
              console.log(
                "Guardian indexes:",
                Array.from(guardianStore.indexNames)
              );
    ...
    const setupRealtime = async (): Promise<boolean> => {
      ...
      try {
        ...
        console.log(
          "🔌 [ScanPage] Connecting to WebSocket with companyId:",
          companyId
        );
        ...
        const unsubscribe = websocketService.subscribeToAttendance((data) => {
          console.log(
            "📺 [ScanPage] Received WebSocket attendance update:",
            data
          );
          ...
          setStats((prev) => ({
            ...
            checkIns:
              newRecord.action === "check_in"
                ? prev.checkIns + 1
                : prev.checkIns,
            checkOuts:
              newRecord.action === "check_out"
                ? prev.checkOuts + 1
                : prev.checkOuts,
            lastScanTime:
              newRecord.timestamp instanceof Date
                ? newRecord.timestamp
        ...
        try {
          const apiRecent = await attendanceService.current.getTodayAttendance(
            100
          );
          if (apiRecent && apiRecent.length > 0) {
            setRecentScans(apiRecent);
            console.log(
              `✅ [ScanPage] Loaded ${apiRecent.length} records from API`
            );
          } else {
          ...
          console.error(
            "❌ [ScanPage] Failed to load initial API data:",
            error
          );
    ...
    const initializeAll = async () => {
      ...
      try {
        ...
        if (!realtimeSuccess) {
          console.log(
            "⚠️ [ScanPage] Realtime not available, enabling polling fallback..."
          );
          refreshInterval = setInterval(() => {
            // Only poll if realtime is still not connected
            if (!realtimeConnected) {
              console.log(
                "🔄 [ScanPage] Periodic refresh of recent scans (realtime not connected)..."
              );
              loadRecentScans();
            }
          }, 5000);
  ...
  useEffect(() => {
    ...
    if (realtimeConnected) {
      ...
      console.log(
        "🔴 [ScanPage] Realtime disconnected, starting polling fallback"
      );
      // Start polling as fallback
      if (!pollInterval) {
        pollInterval = setInterval(() => {
          console.log(
            "🔄 [ScanPage] Polling for updates (realtime disconnected)..."
          );
          loadRecentScans();
        }, 5000);
      }
      ...
      try {
        ...
        if (apiRecent && apiRecent.length > 0) {
          ...
          setRecentScans((prev) => {
            ...
            if (
              prev.length !== apiRecent.length ||
              (prev.length > 0 &&
                apiRecent.length > 0 &&
                prev[0].id !== apiRecent[0].id)
            ) {
              console.log(
                `📊 [ScanPage] Updated recent scans: ${apiRecent.length} records`
              );
              return apiRecent;
            }
            return prev;
  ...
  const handleScan = useCallback(async (data: string) => {
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        const { dbManager } = await import("@/lib/db/db-manager");
        if (type === "student") {
          const allStudents = await dbManager.getAll<Student>(
            "students",
            undefined,
            5
          );
          ...
          const allGuardians = await dbManager.getAll<Guardian>(
            "guardians",
            undefined,
            5
          );
      ...
      const record = await attendanceService.current.recordAttendance(
        data,
        personData
      );
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="mt-4 space-y-2">
              <div className="rounded-lg bg-blue-50 p-3">
                <p className="text-sm text-blue-800">
                  <strong>Offline Mode:</strong> All scans are saved locally and
            ...
            {scanStatus && (
              <div
                className={`mt-4 rounded-lg p-4 animate-in fade-in slide-in-from-bottom-2 ${
                ...
                <div className="flex items-center justify-between">
                  ...
                  <p className="text-sm font-medium text-gray-600">Last Scan</p>
                    <p className="text-lg font-bold" suppressHydrationWarning>
                      {stats.lastScanTime
                        ? format(stats.lastScanTime, "h:mm:ss a")
                        : "No scans"}
                    </p>
                  </div>
                  ...
                  recentScans.map((scan, index) => (
                    <div
                      ...
                      className={`flex items-center justify-between rounded-lg border p-4 transition-all ${
                        index === 0 &&
                        new Date(scan.timestamp).getTime() > Date.now() - 5000
                          ...
                          <img
                            src={
                              ...
                              `https://ui-avatars.com/api/?name=${encodeURIComponent(
                                ...
                                scan.personType === "student"
                                  ? "3b82f6"
                                  : "10b981"
                              }&color=ffffff&size=128&font-size=0.5&rounded=true`
                            }
                            alt={scan.personName}
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            {scan.personType === "student"
                              ...
                              : "Guardian"}
                            {scan.location && (
                              <span className="ml-2">• {scan.location}</span>
                            )}
                          </p>
                        </div>
                      </div>
