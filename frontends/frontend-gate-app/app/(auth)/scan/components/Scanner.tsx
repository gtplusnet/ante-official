...
interface ScannerProps {
  ...
  onToggleMode?: () => void;
  onCameraChange?: (useFront: boolean) => void;
}
...
export const Scanner = forwardRef<ScannerHandle, ScannerProps>(({ onScan, isActive, useFrontCamera = false, manualInputMode = false, onToggleMode, onCameraChange }, ref) => {
    ...
    const startScanner = async () => {
      try {
        ...
        try {
          ...
          const cameraConstraint = useFrontCamera ? { facingMode: "user" } : { facingMode: "environment" };
  ...
  if (manualInputMode) {
    ...
    return (
      <div className="relative h-full w-full flex flex-col items-center justify-center bg-gray-50">
        {/* Mode toggle button */}
        {onToggleMode && (
          <button onClick={onToggleMode} className="absolute top-4 right-4 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700 transition-colors flex items-center gap-2">
        ...
        <div className="w-full max-w-md px-4">
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-gray-600 mt-2">Enter QR code value in format: student:id or guardian:id</p>
          </div>
          ...
          <form onSubmit={handleManualSubmit} className="space-y-4">
            ...
            <button type="submit" className="w-full bg-blue-600 text-white py-3 px-4 rounded-lg hover:bg-blue-700 transition-colors font-medium text-lg">
              Submit Scan
            </button>
          </form>

          <div className="mt-6 text-center">
            <p className="text-xs text-gray-500">Tip: You can also press Enter to submit</p>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="relative h-full w-full flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          {onToggleMode && (
            <button onClick={onToggleMode} className="mt-4 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700 transition-colors flex items-center gap-2 mx-auto">
      ...
      <div className="relative h-full w-full flex items-center justify-center bg-gray-50">
        <div className="text-center px-4">
          <div className="mb-4">
            ...
            {cameraError && <p className="text-sm text-red-600 mb-4 bg-red-50 px-4 py-2 rounded-lg">{cameraError}</p>}
            <p className="text-sm text-gray-600 mb-4">This could be because:</p>
          ...
          {onToggleMode ? (
            <button onClick={onToggleMode} className="bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700 transition-colors flex items-center gap-2 mx-auto">
              <Keyboard className="w-5 h-5" />
              <span>Use Manual Input Instead</span>
            </button>
          ) : (
            <p className="text-sm text-gray-500 mt-4">Please check your camera settings and reload the page</p>
          )}
        </div>
      </div>
    );
  ...
  return (
    <div className={`relative h-full w-full qr-scanner-container ${useFrontCamera ? "front-camera" : ""}`}>
      <div id="qr-reader" ref={scannerRef} className="h-full w-full flex items-center justify-center"></div>

      {/* Mode toggle button */}
      {onToggleMode && (
        <button onClick={onToggleMode} className="absolute top-4 left-4 bg-blue-600/70 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700 transition-colors flex items-center gap-2">
          <Keyboard className="w-4 h-4" />
      ...
      {!manualInputMode && onCameraChange && (
        <div className="absolute top-4 right-4 text-white rounded-lg text-sm camera-selection-control">
          <div className="relative px-3 py-2 w-full h-full flex justify-center items-center rounded-lg bg-gray-600/70 gap-2">
            <Camera className="w-4 h-4" />
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={useFrontCamera} onChange={(e) => onCameraChange(e.target.checked)} className="absolute top-0 left-0 w-full h-full bg-transparent opacity-0 cursor-pointer" />
              <span>{useFrontCamera ? "Front Camera" : "Rear Camera"}</span>
            </label>
          </div>
        </div>
      )}
